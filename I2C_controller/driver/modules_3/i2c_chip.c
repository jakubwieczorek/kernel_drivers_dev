...
static struct i2c_device_id chip_idtable[] = {
	{ "chip", 0 }, // echo chip 0x04 > new_device
	{}
};
...
// This is the function pointer to the drivers probe routine, which is called when the device and driver are both found on the system
static int chip_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	printk("chip_probe");
